...
- **The mouse option is NOT on.** The sibling mention spec §1/§3.8 states "the app-level `tea.WithMouseCellMotion()` is already on (app.go:63 — the slash S1 prerequisite)." That is inaccurate: `cmd/yolo/main.go:768` is `tea.NewProgram(app)` with **no** `tea.WithMouse*` option, and `app.go:63` is the `routeHome`/`routeSession` const block. The option is a shared prerequisite that the **slash epic's S1** lands first (map order: slash → mention → palette); the palette's mouse slice routes `MouseMsg` to the palette only after that. (See §6.)
...
- **Mouse.** None: `main.go:768` `tea.NewProgram(app)` has no `tea.WithMouse*`; zero `MouseMsg` handling anywhere. (The shared prerequisite the slash epic's S1 lands.)
...
**Order constraint:** S1 (dim) first — it is the `viewModal` change the other slices render on. S2/S3 (buckets/filter) are independent of S1. S4 (mouse) depends on the shared `tea.WithMouseCellMotion()` option (slash S1; or added here if the slash epic hasn't landed). S6 last.
