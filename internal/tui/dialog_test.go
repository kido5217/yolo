...
func TestModalFrameLayout(t *testing.T) {
	...
	// homeView frame clamped to modalChromeMin() = 14 > 24/4 = 6 → panelTop
	// = 14; the panel top-padding line → "Model" on line 15, "  loading…" on
	// line 16 (the home frame's top spacer pushes the panel one row down).
	if want := strings.Repeat(" ", 10) + "Model"; !strings.HasPrefix(stripANSI(lines[15]), want) {
		t.Fatalf("line 15 = %q, want prefix %q", stripANSI(lines[15]), want)
	}
	if want := strings.Repeat(" ", 10) + "  loading…"; !strings.HasPrefix(lines[16], want) {
		t.Fatalf("line 16 = %q, want prefix %q", lines[16], want)
	}
