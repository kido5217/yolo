...
// FakeFromEnv resolves YOLO_LLM/YOLO_FAKE_SCRIPT (M5 gate); env nil = real
// environment (the Loader.Env convention):
...
func FakeFromEnv(env map[string]string) (*fake.Driver, error) {
	...
	script, _ := lookup("YOLO_FAKE_SCRIPT")
	if script == "" {
